...
export default function Day({ day, rowIdx, filteredEvents }) {
  ...
  useEffect(() => {
    const events = filteredEvents.filter((evt) => dayjs(evt.date).subtract(1, 'month').format('DD-MM-YY') === day.format('DD-MM-YY'))
    setDayEvents(events)
  }, [filteredEvents, day])

  function getCurrentDayClass() {
    return day.format('DD-MM-YY') === dayjs().subtract(1, 'month').format('DD-MM-YY') ? 'bg-cyan-600 px-2 py-1 text-white rounded-full' : ''
  }
  return (
    <div className="flex flex-col items-center overflow-y-scroll border border-[#a0a0a040] shadow-lg">
